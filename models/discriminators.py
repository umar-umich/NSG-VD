...
class ClipDiscriminator(nn.Module):
    def __init__(self, seq_len=4, input_dim=512, hidden_dim=256, feature_dim=300):
...
class I3D(nn.Module):
    def __init__(self, input_channels=3, hidden_features=[32, 64, 128], output_dim=300):
        """
        """
        super(I3D, self).__init__()
        self.conv1 = nn.Conv3d(input_channels, hidden_features[0], kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1))
        self.bn1 = nn.BatchNorm3d(hidden_features[0])
        self.pool1 = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2))
        
        self.conv2 = nn.Conv3d(hidden_features[0], hidden_features[1], kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1))
        self.bn2 = nn.BatchNorm3d(hidden_features[1])
        self.pool2 = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2))
        
        self.conv3 = nn.Conv3d(hidden_features[1], hidden_features[2], kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1))
        self.bn3 = nn.BatchNorm3d(hidden_features[2])
        self.pool3 = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2))
        
        self.global_pool = nn.AdaptiveAvgPool3d((1, 1, 1))
        
        self.fc = nn.Linear(hidden_features[2], output_dim)
...
class I3DDiscriminator(nn.Module):
    def __init__(self, input_channels, hidden_features, feature_dim=300):
        super(I3DDiscriminator, self).__init__()
        
        self.i3d = I3D(input_channels=input_channels, hidden_features=hidden_features, output_dim=feature_dim)
    ...
    def forward(self, x, out_feature=False):
        i3d_feature = self.i3d(x)  # (B, 512)
        # fc_feature = self.fc(i3d_feature)
        validity = self.adv_layer(i3d_feature)
        
        if out_feature:
            return validity, i3d_feature
        else:
            return validity
...
class SWINDiscriminator(nn.Module):
	def __init__(self, feature_dim=300):
		...
		self.dim_embed = 1024
		self.encoder = TALL_SWIN(pretrained=True)
		self.mlp = nn.Linear(self.dim_embed, feature_dim)
		trunc_normal_(self.mlp.weight, std=.02)
		if isinstance(self.mlp, nn.Linear) and self.mlp.bias is not None:
	...
	def forward(self, x, out_feature):
		feature = self.encoder.forward_features(x) # shape = (B, 1024)
		feature = self.mlp(feature) # shape = (B, feature_dim)
...
class MLPDiscriminator(nn.Module):
	def __init__(self, input_dim=8, num_features=300):
		super(MLPDiscriminator, self).__init__()
		self.feature_dim = num_features
		self.mlp = nn.Linear(input_dim, num_features)
		trunc_normal_(self.mlp.weight, std=.02)
		if isinstance(self.mlp, nn.Linear) and self.mlp.bias is not None:
			nn.init.constant_(self.mlp.bias, 0)
		self.adv_layer = nn.Sequential(
	...
	def forward(self, x, out_feature):
		feature = self.mlp(x) # shape = (B, 1024)
