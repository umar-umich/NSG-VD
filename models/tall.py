...
#            trunc_normal_(self.absolute_pos_embed, std=.02)
    ...
    def create_thumbnail(self, x):
        ...
        x = rearrange(x, 'b (th tw c) h w -> b c (th h) (tw w)', th=self.thumbnail_rows, c=self.in_chans)
        return x

    def pad_frames(self, x):
        frame_num = self.duration - self.frame_padding
        x = x.view((-1,self.in_chans*frame_num)+x.size()[2:])
        x_padding = torch.zeros((x.shape[0], self.in_chans*self.frame_padding) + x.size()[2:]).cuda()
        x = torch.cat((x, x_padding), dim=1)
        assert x.shape[1] == self.in_chans * self.duration, 'frame number %d not the same as adjusted input size %d' % (x.shape[1], self.in_chans * self.duration)
...
#        x = rearrange(x, 'b (t c) h w -> b c h (t w)', t=self.duration)
        ...
        if self.frame_padding > 0:
            ...
            x = x.view((-1,self.in_chans*self.duration)+x.size()[2:])

        if self.image_mode:
            x = self.create_thumbnail(x) 
            x = nn.functional.interpolate(x, size=self.img_size,mode='bilinear')
        else:
            x = rearrange(x, 'b (n t c) h w -> (b n t) c h w', t=self.duration, c=self.in_chans)
...
#           x = x + self.absolute_pos_embed
    ...
    def forward_tokens(self, x):
        b, t, _, h, w = x.shape
        x = x.contiguous().view(b, t*self.in_chans, h, w)
        if self.frame_padding > 0:
            x = self.pad_frames(x)
        else:
            x = x.view((-1,3*self.duration)+x.size()[2:])
...
#           x = x + self.absolute_pos_embed
    ...
    def forward(self, x):
        x = self.forward_features(x) # (batch_size, embed_dim=1024)
        x = self.head(x) # (batch_size, num_classes=1)
...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, img_size=224, num_patches=196,
    ...
    if len(pretrained_model) == 0:
        ...
        try:
            ...
            state_dict = load_state_dict_from_url(pretrained_model)
...
class TripleTALL_SWIN(SwinTransformer):
    def __init__(self, *args, **kwargs):
        super(TripleTALL_SWIN, self).__init__(*args, **kwargs)
        self.embed_dim = 1024
        self.num_classes = 1
        self.encoders = nn.ModuleList([TALL_SWIN(pretrianed=True) for i in range(3)])
        self.decoder = nn.Sequential(
            nn.Linear(3*self.embed_dim, self.num_classes),
            # nn.ReLU(),
    ...
    def forward(self, x):
        """
        x's shape = (B, T, C=9, W=224, H=224)
        output's shape = (B, num_classes=1)
        """
        features = []
        for i in range(3):
            feature = self.encoders[i].forward_features(x[:,:,3*i:3*(i+1),:,:]) # input's shape = (B, T, C=3, W, H)
            features.append(feature) # featur's shape = (B, embed_dim=1024)
        features = torch.concat(features, dim=1) # features' shape = (B, 3*embed_dim=3072)
        output = self.decoder(features) 
        return output
        
class CrossAttnTripleTALL_SWIN(nn.Module):
    def __init__(self, *args, **kwargs):
        super(CrossAttnTripleTALL_SWIN, self).__init__(*args, **kwargs)
        self.embed_dim = 1024
        self.num_classes = 1
        self.q_encoder = TALL_SWIN()
        self.k_encoder = TALL_SWIN()
        self.v_encoder = TALL_SWIN()
        self.cross_attention = nn.MultiheadAttention(embed_dim=self.embed_dim, num_heads=8, batch_first=True)
        self.decoder = nn.Sequential(
            nn.Linear(self.embed_dim, self.num_classes),  # Linear layer to map to num_classes
        )
    ...
    def forward(self, x):
        ...
        output's shape = (B, num_classes=1)
        """
        # input's shape = (B, T, C=3, W, H)
        q_feature = self.k_encoder.forward_tokens(x[:,:,0:3,:,:]) # feature's shape = (B, #tokens, embed_dim=1024)
        k_feature = self.q_encoder.forward_tokens(x[:,:,3:6,:,:]) # feature's shape = (B, #tokens, embed_dim=1024)
        v_feature = self.v_encoder.forward_tokens(x[:,:,6:9,:,:]) # feature's shape = (B, #tokens, embed_dim=1024)
        
        # Apply cross-attention
        attn_output, _ = self.cross_attention(q_feature, k_feature, v_feature)  # (B, #tokens, embed_dim)
        attn_output = attn_output.mean(dim=1)  # (B, embed_dim)
        output = self.decoder(attn_output)  # (B, num_classes=1)
        return output
 
class GatedTripleTALL_SWIN(nn.Module):
    def __init__(self, *args, **kwargs):
        super(GatedTripleTALL_SWIN, self).__init__(*args, **kwargs)
        self.embed_dim = 1024
        self.num_classes = 1
        self.q_encoder = TALL_SWIN()
        self.k_encoder = TALL_SWIN()
        self.v_encoder = TALL_SWIN()
        self.gated_layer = nn.Linear(3*self.embed_dim, 3)
        self.decoder = nn.Linear(self.embed_dim, self.num_classes)  # Linear layer to map to num_classes
    ...
    def forward(self, x):
        ...
        """
        # input's shape = (B, T, C=3, W, H)
        q_feature = self.k_encoder.forward_features(x[:,:,0:3,:,:]) # feature's shape = (B, embed_dim=1024)
        k_feature = self.q_encoder.forward_features(x[:,:,3:6,:,:]) # feature's shape = (B, embed_dim=1024)
        v_feature = self.v_encoder.forward_features(x[:,:,6:9,:,:]) # feature's shape = (B, embed_dim=1024)

        gated_weights = self.gated_layer(torch.cat((q_feature, k_feature, v_feature), dim=1))  # (B, 3)
        gated_weights = torch.softmax(gated_weights, dim=1)
        
        fused_features = (
            gated_weights[:, 0:1] * q_feature +  # g1 * Q
            gated_weights[:, 1:2] * k_feature +  # g2 * K
            gated_weights[:, 2:3] * v_feature    # g3 * V
        )   # (B, 3*embed_dim)
        output = self.decoder(fused_features)  # (B, num_classes=1)
        return output

class SingleSwinBlockDiscriminator(nn.Module):
    def __init__(self, num_features=300, duration=8):
        super().__init__()
        self.encoder = SwinTransformer(depths=[2],num_heads=[3],num_classes=num_features, duration=duration)
    ...
    def forward(self, x, out_feature=False):
        ...
        x: (B, T, C=3, H, W)
        """
        # (B, n* 4096) -> (B, num_features=300)
        feature = self.encoder(x) # （B, num_features=300)
...
class Edm2SwinBlockDiscriminator(nn.Module):
    def __init__(self, num_features=300, duration=8):
        super().__init__()
        self.encoder = SwinTransformer(depths=[2],num_heads=[3],in_chans=4,num_classes=num_features, duration=duration, img_size=64, window_size=4)
...
class SingleSwinBlock(nn.Module):
    def __init__(self, depths=[2], num_heads=[4], num_features=300):
        super().__init__()
        hidden_dim = 1000
        self.encoder = SwinTransformer(depths=depths, num_heads=num_heads,num_classes=hidden_dim)
